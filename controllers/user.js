...
export const register = async (req, res) => {
    ...
    //Generara los altos para encriptar
    const salt = await bcrypt.genSalt(10);

    //Enriptar la contraseña y guaradarla en hashedPassword
    const hashedPassword = await bcrypt.hash(user_to_save.password, salt);

    //asignarla contrasña encriptada al objeto del usuario
  ...
  export const login = async (req, res) =>{
    try {
      ...
      //Buscar en la base de datos si existe el email registrado
      ...
      //si la contraseña es incorrecta (flase)
      if(!validPassword){
      ...
      //Generara el token de autenticacion (JWT)
      ...
      console.log("error en la autenticacion del ususario: ")
      //Devolver mensaje de error
      return res.status(500).send({
        status: "error",
        message: "Error en la autenticaion del usuario"
      })
